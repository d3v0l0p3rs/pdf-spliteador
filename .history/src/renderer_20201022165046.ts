...
const getPDFDictionary = (): { [pdfName: string]: PDFDocument } => {
    ...
    const linesNumber = getCurrentLines().length
    getCurrentLines().forEach(async (line, i) => {
        if (!pdfDictionary.hasOwnProperty(line)) { pdfDictionary[line] = await PDFDocument.create() }
        const [copiedPage] = await pdfDictionary[line].copyPages(pdf, [i])
        pdfDictionary[line].addPage(pdf.addPage(copiedPage))
        if (i === linesNumber-1) { createPDFFiles(pdfDictionary) }
    });
