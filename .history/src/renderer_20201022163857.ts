...
const exportPDF = async() => {
    // writeFileSync('myfile.pdf', Buffer.from(await pdf.save()));
    for (const [pdfName, pdfFile] of Object.entries(getPDFDictionary())) {
        console.log(pdfName)
        writeFileSync('pdf/' + pdfName, Buffer.from(await pdfFile.save()));
    }
    console.log(getPDFDictionary())
}

const getPDFDictionary = (): { [pdfName: string]: PDFDocument } => {
    let pdfDictionary: {[pdfName: string]: PDFDocument } = {}
    await getCurrentLines().forEach(async (line, i) => {
        if (!pdfDictionary.hasOwnProperty(line)) { pdfDictionary[line] = await PDFDocument.create() }
        // console.log(pdf)
        pdfDictionary[line].copyPages(pdf, [i])
        // pdfDictionary[line].addPage(pdf.getPage(i))
    });
