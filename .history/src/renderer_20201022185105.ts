...
const toggleGeneratingPDFMessage = (toggle: boolean) => {
    setProgressBar(0);
    if (toggle) {
        return (<any>$('#generating-pdfs-modal')).modal('show')
    } else {
        return setTimeout(_ => (<any>$('#generating-pdfs-modal')).modal('hide'), 500)
    }
}
...
const setProgressBar = (percentage: number) => {
    ...
    if (currentValue !== undefined && currentValue !== null && currentValue !== percentage) {
        ...
        (<any>$('.progress-bar')).css('width', (100-percentage)+'%').attr('aria-valuenow', (100-percentage));
    }
    console.log("-----------------------")
