...
const updateMessages = () => {
    ...
    const lines = nameLinesText.value.split(/\r\n|\r|\n/).length
    const linesDiff = lines - expectedLines;
    uploadFileWarning.style.display = pdf ? "none" : "block"
    console.log(linesDiff)
    notLineMatchWarning.style.display = linesDiff === 0 ? "none" : "block"
    if (linesDiff !== 0) {
        if (linesDiff > 0) {
            notLineMatchWarning.innerHTML = "volado"
        } else {
            notLineMatchWarning.innerHTML = "lefalta"
        }
    }
}
