...
const packagePDFs = () => {
    let pdfDictionary: {[key: string]: PDFPage[]}  = {}
    getCurrentLines().forEach((line, i) => {
        if (!pdfDictionary.hasOwnProperty(line)) { pdfDictionary[line] = [] }
        pdfDictionary[line].push(pdf.getPage(i))
    });
    console.log(pdfDictionary)
}
