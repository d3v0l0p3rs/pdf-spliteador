...
const updateMessages = () => {
    ...
    const blankLinesCheck = nameLinesText.value.split(/\r\n|\r|\n/).reduce((a, c) => a &&!!c, true)
...
uploadPDFInput.addEventListener("change", async() => {
    ...
    fs.readFile(pdf.path, 'utf8', function(err, data){ 
      
        // Display the file content 
        console.log(data); 
    }); 
      
    const existingPdfBytes = await fetch(pdf.path).then(res => res.arrayBuffer())
